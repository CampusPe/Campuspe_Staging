...
async function checkUsers() {
  try {
    ...
    const recruiters = await User.find({ role: 'recruiter' }).select('email firstName lastName role status isVerified password');
    console.log('\nRecruiters found:');
    recruiters.forEach(r => console.log(`- ${r.email} | Status: ${r.status} | Verified: ${r.isVerified} | HasPassword: ${!!r.password}`));
