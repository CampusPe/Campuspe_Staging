...
const createCollege = async (req, res) => {
    try {
        const { userId, collegeName, collegeType, establishedYear, recognizedBy, website, affiliatedTo, aboutCollege, address, location, landmark, pincode, city, state, coordinatorName, coordinatorDesignation, coordinatorNumber, coordinatorEmail, email, phoneNumber } = req.body;
        if (!userId || !collegeName || !email) {
            return res.status(400).json({
                message: 'Missing required fields: userId, collegeName, and email are required'
            });
        }
        const existingCollege = await College_1.College.findOne({ userId });
        if (existingCollege) {
            return res.status(400).json({
                message: 'College profile already exists for this user'
            });
        }
        const generateDomainCode = () => {
            const code = collegeName
                .toUpperCase()
                .replace(/[^A-Z]/g, '')
                .substring(0, 4) + Math.floor(Math.random() * 10000);
            return code;
        };
        const collegeData = {
            userId,
            name: collegeName,
            shortName: collegeName.split(' ').map((word) => word[0]).join('').toUpperCase(),
            ...
            primaryContact: {
                name: coordinatorName || 'Administrator',
                designation: coordinatorDesignation || 'Administrator',
                email: coordinatorEmail || email,
                phone: coordinatorNumber || phoneNumber || ''
            },
        ...
        res.status(201).json({
            ...
            college: {
                ...
                name: savedCollege.name,
                domainCode: savedCollege.domainCode,
                approvalStatus: savedCollege.approvalStatus
            }
        });
    }
    catch (error) {
        ...
        if (error instanceof Error && 'code' in error && error.code === 11000) {
            ...
            if (field === 'domainCode') {
                return res.status(400).json({
                    message: 'Domain code already exists. Please try again.'
                });
            }
            return res.status(400).json({
                message: `Duplicate ${field}: A college with this ${field} already exists`
            });
        }
        res.status(500).json({
            message: 'Server error creating college',
